...
export const verifyLdapUser = (username: string, password: string, req: $RequestExtend) => {
  ...
  return new Promise((res, rej) => {
    ...
    return client.bind(
      'cn=autotest,ou=people,dc=shuyun,dc=com',
      'Shuyun789',
        ...
        if (err) {
          ...
          return client.search('DC=shuyun,DC=com', opts, function (_err, response) {
            ...
            response.on('searchEntry', function (entry) {
              ...
              if (!realPasswd || typeof realPasswd !== 'string') {
                ...
                if (temp !== null) {
                  ...
                  const inputPwd = Buffer.from(
                    crypto.createHash(cipher).update(inputData).digest('base64'),
                    'base64'
                  );
            ...
            response.on('error', function (err) {
              ...
              searchSuccess = false;
              req.logger.error(
                'verifyLdapUser response.on error: ' + err.message,
                'username',
                username
              );
              client.unbind();
